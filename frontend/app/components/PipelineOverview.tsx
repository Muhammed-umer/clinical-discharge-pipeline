...
const PIPELINE_STAGES = [
  ...
  { id: 'summary', name: 'Summary', desc: 'Synthesizes verified structured findings into final discharge format.' }
];
          ...
          <div className="space-y-2 relative">
            ...
            {PIPELINE_STAGES.map((stage, idx) => {
              ...
              if (isConflictDiscrepancyStep) {
                ...
                iconClass = 'bg-rose-950 border-rose-800 text-rose-455';
              }
              ...
              return (
                ...
                >
                  <div className={`w-8 h-8 rounded-full border flex items-center justify-center font-mono text-xs font-bold shrink-0 ${iconClass}`}>
                    {statusText === 'Completed' ? '✓' : statusText === 'Running' ? '⚡' : statusText.includes('Conflict') ? '⚠️' : idx + 1}
                  </div>

                  <div className="space-y-0.5">
                    <div className="flex flex-wrap items-center gap-2">
                      <span className="font-mono text-xs font-bold text-slate-200">{stage.name}</span>
